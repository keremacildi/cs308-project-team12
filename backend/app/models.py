...
class DiscountCampaign(models.Model):
    name = models.CharField(max_length=100, unique=True)
    products = models.ManyToManyField(Product, related_name='discount_campaigns')
    ...
    def __str__(self):
        return f"Discount Campaign: {self.name}"


# -------------------------------------------
# SPRINT 5 Model: ReturnRequest
# -------------------------------------------
class ReturnRequest(models.Model):
    ...
    order_item = models.OneToOneField(
        ...
        help_text="Which specific item the customer wants to return."
    )
    request_date = models.DateTimeField(auto_now_add=True)
    status = models.CharField(max_length=20, choices=REQUEST_STATUS_CHOICES, default='pending')
    reason = models.TextField(blank=True, null=True, help_text="Reason for return.")
    refund_amount = models.DecimalField(max_digits=10, decimal_places=2, default=0.00, help_text="Amount to be refunded if approved.")

    def __str__(self):
        return f"Return Request for {self.order_item.product.name} (Order #{self.order_item.order.id})"
