...
# Uncomment the following if you are using Django REST Framework endpoints
...
def add_to_cart(request, product_id):
    """
    Adds a product to the shopping cart, either for an authenticated user or via session.
    """
    product = get_object_or_404(Product, pk=product_id)
...
def checkout(request):
    ...
    - Creates an Order (and corresponding OrderItems).
    - Forwards the order for delivery processing by creating a Delivery record.
    ...
    # Email the invoice with PDF attachment.
...
def submit_rating(request, product_id):
    """
    Allows a user to submit a rating (1-5) for a product only if they have a delivered order for it.
    """
...
def submit_comment(request, product_id):
    ...
    The comment is created as not approved by default.
    """
    product = get_object_or_404(Product, id=product_id)
    has_delivered = OrderItem.objects.filter(
        order__user=request.user,
...
def send_invoice_email(order):
    """
    Sends an email with the PDF invoice attached.
    """
...
def merge_cart(sender, request, user, **kwargs):
    """
    Merges shopping cart items saved under the session with the logged-in user's cart.
    If the same product exists in both, their quantities are added together.
    """
    session_key = request.session.session_key
    if not session_key:
        return
